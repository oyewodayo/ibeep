...
import { Star, Clock, Users, Award, ChevronRight, Play, BookOpen, Code, Brain, Cloud, Smartphone, Globe, ArrowRight } from 'lucide-react'; // Added ArrowRight for a subtle improvement
import Navigation from '@/components/Navigation';
import Footer from '@/components/Footer';
import { Button } from '@/components/ui/button';
import gsap from 'gsap';
import { ScrollTrigger } from 'gsap/ScrollTrigger';
import { TextPlugin } from 'gsap/TextPlugin'; // Import TextPlugin for typewriter effect

gsap.registerPlugin(ScrollTrigger, TextPlugin); // Register TextPlugin
...
const MentorshipOne = () => {
  ...
  const taglineRef = useRef(null); // Ref for dynamic tagline
  ...
  useEffect(() => {
    ...
    const taglines = ["AI/ML", "Cloud & DevOps", "Mobile Development", "Web Development"];
    let currentTaglineIndex = 0;

    const typeText = () => {
      const currentElement = taglineRef.current;
      ...
      gsap.to(currentElement, {
        ...
        onComplete: () => {
          gsap.to(currentElement, {
            duration: 0.5,
            delay: 2, // Pause after typing
            text: "", // Erase text
            ease: "none",
            onComplete: () => {
              currentTaglineIndex = (currentTaglineIndex + 1) % taglines.length;
              typeText(); // Start typing next tagline
            }
          });
        }
      });
    ...
    return () => {
      ...
      gsap.killTweensOf(taglineRef.current); // Clean up typewriter animation
    };
  }, []);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900">
      ...
      {/* Hero Section */}
      <section className="relative pt-24 pb-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-purple-950/20 overflow-hidden">
        {/* Subtle background circles/patterns */}
        <div className="absolute top-0 left-0 w-48 h-48 bg-blue-300 dark:bg-blue-700 rounded-full opacity-10 animate-blob mix-blend-multiply filter blur-xl"></div>
        <div className="absolute bottom-0 right-0 w-64 h-64 bg-purple-300 dark:bg-purple-700 rounded-full opacity-10 animate-blob animation-delay-2000 mix-blend-multiply filter blur-xl"></div>
        <div className="absolute top-1/4 right-1/4 w-32 h-32 bg-teal-300 dark:bg-teal-700 rounded-full opacity-10 animate-blob animation-delay-4000 mix-blend-multiply filter blur-xl"></div>


        <div className="max-w-7xl mx-auto relative z-10"> {/* Added relative z-10 */}
          <div ref={heroRef} className="text-center">
            <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl mb-8 shadow-lg animate-bounce-slow"> {/* Added subtle bounce animation */}
            ...
            <p className="text-2xl text-blue-700 dark:text-blue-300 font-semibold max-w-3xl mx-auto mb-4 h-8 overflow-hidden">
              <span ref={taglineRef} className="inline-block"></span><span className="animate-blink">|</span> {/* Cursor for typewriter effect */}
            </p>

            <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto mb-8 leading-relaxed">
              Get personalized mentorship from industry experts at top tech companies.
              Master your chosen field with hands-on guidance.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-12">
              ...
              <Button variant="outline" className="px-8 py-4 text-lg font-semibold rounded-xl border-2 border-gray-300 dark:border-gray-600 text-gray-800 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center group">
                Explore Tracks <ArrowRight className="ml-2 w-5 h-5 transition-transform duration-200 group-hover:translate-x-1" />
              </Button>
            </div>

            {/* Testimonial Snippet */}
            <div className="mt-12 bg-white dark:bg-gray-800/70 p-6 rounded-xl shadow-inner max-w-xl mx-auto">
              <p className="text-gray-700 dark:text-gray-300 italic mb-4">
                &quot;My mentor helped me land my dream job at Google! The personalized roadmap and weekly code reviews were invaluable.&quot;
              </p>
              <div className="flex items-center justify-center">
                <img src="https://api.dicebear.com/8.x/initials/svg?seed=JD" alt="John Doe" className="w-10 h-10 rounded-full mr-3 border-2 border-blue-400" /> {/* Placeholder avatar */}
                <span className="font-semibold text-gray-900 dark:text-white">John D., Software Engineer at Google</span>
              </div>
            </div>
